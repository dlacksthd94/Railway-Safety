...
def get_acc_table(path_form57_csv, path_dict_col_indexing, path_dict_idx_mapping, path_dict_answer_places, df_match, list_answer_type_selected, config):
    ...
    if json_source in ['img', 'pdf']:
        dict_idx_answer_type = {
            ...
            'free-text': ['1', '2', '3', '5', '6', '7', '9', '11', '12', '20c_name', '20c_measure', '26'],
        ...
        dict_idx_answer_type = {
            ...
            'free-text': {'1': 'Railroad Name', '5': 'Other Railroad Name', '9': 'Maintenance Railroad Name', '21': 'Nearest Station', '24': 'County Name', '26': 'City Name', '46': 'Track Name', '47': 'Track Class'},
    ...
    for idx_match, row_match in tqdm(df_match.iterrows(), total=df_match.shape[0]):
        ...
        for col_idx_json, col_idx_form  in dict_idx_selected.items():
            ...
            if not (isinstance(retrieval, str) and retrieval.lower() == 'unknown') and pd.notna(retrieval) and pd.notna(label):
                if col_idx_form in dict_idx_answer_type['free-text']:
